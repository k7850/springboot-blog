...
public class BoardRepository {
    ...
    public Integer count(String keyword, Integer keywordUserId){
        Query query = em.createNativeQuery("select count(*) from board_tb where title like :keyword or user_id like :keywordUserId");
    ...
    public List<Board> findAll(int page, String keyword, Integer keywordUserId){
        ...
        Query query = em.createNativeQuery("select * from board_tb where user_id like :keywordUserId or title like :keyword order by CASE WHEN user_id LIKE :keywordUserId THEN 1 ELSE 2 END, id desc limit :page, :size", Board.class);
            query.setParameter("keywordUserId", keywordUserId);
